...
import { ArrowLeft, Eye, EyeOff, Mail, Pencil, X } from "lucide-react";
...
interface PostPropertyAuthModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  /** Prefill phone from the post-property form */
  initialPhone?: string;
}
...
export function PostPropertyAuthModal({
  open,
  onOpenChange,
  initialPhone = "",
}: PostPropertyAuthModalProps) {
  ...
  const [role, setRole] = useState<PartnerRole>("owner");
  ...
  const enterPortal = (token: string, user: Parameters<typeof toAuthUser>[0]) => {
    setSession({ token, user: toAuthUser(user) });
    toast.success("Welcome", {
      description: user.uiRole === "agent" ? "Agent Portal" : "Owner Portal",
    });
    onOpenChange(false);
    router.push("/portal");
  };
  ...
  const verifyOtp = async () => {
    ...
    try {
      ...
      if (res.token && res.user && !res.isNewUser) {
        enterPortal(res.token, res.user);
        return;
      }
      setStep("create");
    } catch (err) {
  ...
  const back = () => {
    ...
    else if (step === "create") setStep(otp.join("").length === 4 ? "otp" : "password");
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div
        ...
        className="relative z-10 w-full max-w-[400px] rounded-lg border border-border bg-white p-6 shadow-2xl"
      >
        {step !== "phone" ? (
          <button
            type="button"
            onClick={back}
            className="absolute top-4 left-4 rounded-md p-1 text-muted hover:bg-cream hover:text-charcoal"
            aria-label="Back"
          >
        ...
        {step === "phone" ? (
          <>
            <h2 id="pp-auth-title" className="pr-8 text-xl font-bold text-charcoal">
              Login / Register
            </h2>
            ...
            <div
              className={cn(
                "mt-6 rounded-md border px-3 py-2",
            ...
            <Button
              type="button"
              onClick={goOtp}
              className="mt-5 h-11 w-full rounded-md text-base font-semibold text-white gold-gradient"
            >
              Continue
            </Button>
            ...
            <Button
              ...
              className="mt-3 h-11 w-full gap-2 rounded-md border-border text-charcoal"
            >
        ...
        {step === "email" ? (
          <>
            <h2 id="pp-auth-title" className="px-8 text-xl font-bold text-charcoal">
              Login / Register
            </h2>
            <p className="mt-1 text-sm text-muted">Please enter your Email ID/Username</p>

            <div className="mt-6 rounded-md border border-border px-3 py-2 focus-within:border-gold">
            ...
            {emailError ? <p className="mt-1 text-xs text-danger">{emailError}</p> : null}
        ...
        {step === "password" ? (
          <>
            ...
            <p className="mt-1 text-sm text-muted">
              Your Password for{" "}
              <span className="font-medium text-gold-rich">{email}</span>{" "}
              <button
                type="button"
                onClick={() => setStep("email")}
                className="inline text-gold-rich"
            ...
            <div className="relative mt-6 rounded-md border border-border px-3 py-2 focus-within:border-gold">
        ...
        {step === "create" ? (
          <>
            ...
            <p className="mt-5 text-sm font-medium text-charcoal">You are</p>
            ...
            <div className="mt-4 rounded-md border border-border px-3 py-2 focus-within:border-gold">
            ...
            <div className="mt-3 rounded-md border border-border bg-[#F7F5F0] px-3 py-2">
            ...
            <button
              type="button"
              onClick={() => setStep("phone")}
              className="mt-1.5 text-xs font-medium text-gold-rich hover:underline"
            >
              Change Number
            </button>
